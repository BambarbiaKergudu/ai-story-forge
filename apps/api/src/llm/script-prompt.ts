import { STORY_BEATS, STYLE_IDS, type StyleId } from '@asf/contracts';
...
export function buildScriptMessages(input: ScriptPromptInput): {
  ...
  return {
    system: [
      ...
      `Четыре панели идут строго в порядке beat: ${STORY_BEATS.join(', ')}.`,
      'Это четыре разных видимых события: завязка, препятствие или улика, поворот, развязка.',
      'Каждая следующая сцена меняет место или действие. Не повторяй предыдущий кадр и не копируй его imagePrompt.',
      'Ответь одним JSON-объектом по схеме. Никакого текста вокруг.',
      'caption — на том же языке, что и идея пользователя. Подписи вместе читаются как эта история.',
      ...
      'imagePrompt — по-английски, одна-две фразы: только новое действие и место этого такта. Не повторяй стиль и не описывай внешность персонажа: стиль и паспорт подставятся отдельно.',
      'Текст imagePrompt совпадает с shotType: establishing — место целиком, персонаж мал; wide — персонаж в рост вместе с окружением; full-shot — фигура от макушки до ног; medium — по пояс, жест и мимика; close-up — лицо; extreme-close-up — одна деталь; insert — один предмет.',
