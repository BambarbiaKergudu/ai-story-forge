...
Composition: {shot description}. {camera description}. No text, no speech bubbles, no watermark.
```

Крупность и ракурс в промпт картинки попадают словами (`wide view of the place`,
`camera looking up`), а не сырым id. Текст сцены просим писать в той же крупности.

**Уровень 2 (режим `consistent`).** Первая панель генерируется обычным способом и
становится референсом: её PNG передаётся на вход Gemini Flash Image при генерации
панелей 2–4. Текст перед промптом говорит, что картинка задаёт только лица, тела
и одежду. Позу, место, фон и кадрирование брать из новой сцены.

Спайк (одна история, 12 кадров) выбрал так: черновик — это уровень 1 на FLUX
schnell, паспорт и общий `seed` на всю историю. Голый промпт без паспорта и без
seed в продукт не идёт. Цельный режим — уровень 2, Gemini с референсом первой
панели: персонаж держится заметно лучше. Витринные кадры тоже собираются уровнем 2.

**Валидация ответа LLM.** Схема zod: 4 сцены, у каждой `beat` (`setup`, `obstacle`,
`turn`, `payoff`), непустой `caption`, `imagePrompt` на английском (не короче 20
символов, без верхнего предела), `shotType` из перечисления. После схемы api
проверяет порядок тактов и то, что `imagePrompt` не повторён дословно. При провале —
повтор с ошибкой валидации в промпте. Один и тот же zod-объект используется как
источник для JSON Schema в structured output и как рантайм-валидатор. `beat` в базу
не пишется: панели хранят подпись, сцену, крупность и ракурс.
...
| Draft      | FLUX schnell, паспорт и общий seed    | гостям и всем пользователям | 3 / сутки на гостя, 20 / месяц на юзера |
